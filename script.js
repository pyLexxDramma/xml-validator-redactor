document.addEventListener('DOMContentLoaded', function() {
    ...
    const validateXsdButton = document.getElementById('validateXsd');
    const saveXsdButton = document.getElementById('saveXsd');
    ...
    let xmlEditor = CodeMirror.fromTextArea(document.getElementById('xmlEditor'), {
        mode: "xml",
        theme: "dracula",
        lineNumbers: true,
        lineWrapping: true
    ...
    let xsdEditor = CodeMirror.fromTextArea(document.getElementById('xsdEditor'), {
        mode: "xml",
        theme: "dracula",
        lineNumbers: true,
        lineWrapping: true
    ...
    xsdFileElement.addEventListener('change', function(event) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            reader.onload = function(e) {
                ...
                xsdEditor.setValue(e.target.result); // Отображаем XSD в CodeMirror
    ...
    saveXsdButton.addEventListener('click', function() {
        const selectedXsd = xsdSelectElement.value;
        if (!selectedXsd) {
            showMessage('Ошибка: Выберите XSD схему.', true);
            return;
        }
        xsdSchemas[selectedXsd] = xsdEditor.getValue();
        showMessage('XSD схема сохранена.');
    });

    validateXsdButton.addEventListener('click', function() {
        const xsdString = xsdEditor.getValue();
        validateXsd(xsdString);
    });

    function validateXsd(xsdString) {
        fetch('http://localhost:3000/validateXsd', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            },
            body: `xsd=${encodeURIComponent(xsdString)}`
        })
        .then(response => response.json())
        .then(data => {
            if (data.valid) {
                showMessage('XSD схема валидна.', false);
            } else if (data.errors) {
                showMessage('XSD схема не валидна:\n' + data.errors.join('\n'), true);
            } else {
                showMessage('Ошибка валидации XSD на сервере.', true);
            }
        })
        .catch(error => {
            showMessage('Ошибка при отправке запроса на сервер: ' + error.message, true);
